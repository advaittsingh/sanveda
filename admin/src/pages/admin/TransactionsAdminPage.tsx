...
export default function TransactionsAdminPage() {
  ...
  /** Same shared fetch/mapping as Donations "Download PDF" — never map TransactionRecord fields directly. */
  const exportReceipt = async (donationId: string) => {
    setOpenMenuId('')
    setReceiptBusyId(donationId)
    setActionMessage(null)
    try {
      const { downloadReceiptForDonationId } = await import('../../lib/receipt80G/receipt80GService')
      const { markReceiptDownloaded } = await import('../../lib/donationOperationsService')
      await downloadReceiptForDonationId(donationId)
      try {
        await markReceiptDownloaded(donationId)
      } catch (markErr) {
        console.warn(markErr)
      }
      setActionMessage({ tone: 'ok', text: '80G receipt PDF downloaded.' })
      window.scrollTo({ top: 0, behavior: 'smooth' })
    } catch (err) {
      setActionMessage({
        tone: 'error',
        text: err instanceof Error ? err.message : 'Could not export the 80G receipt.',
      })
      window.scrollTo({ top: 0, behavior: 'smooth' })
    } finally {
  ...
  const exportPdf = async () => {
    if (!dashboard || pdfExportBusy) return
    setPdfExportBusy(true)
    setActionMessage(null)
    try {
      await exportTransactionsPdf(dashboard)
      setActionMessage({ tone: 'ok', text: 'Transactions PDF downloaded.' })
    } catch (err) {
      setActionMessage({
        tone: 'error',
        text: err instanceof Error ? err.message : 'Could not export the transactions PDF.',
      })
    } finally {
  ...
  const headerActions = (
    <>
      <button
        ...
        onClick={() => exportTransactionsCsv(dashboard.filteredTransactions)}
      >
      ...
      <button
        ...
        disabled={pdfExportBusy || !dashboard}
        aria-busy={pdfExportBusy}
      >
        <Download size={14} className="mr-1.5" />
        {pdfExportBusy ? 'Exporting PDF…' : 'Export PDF'}
      </button>
                              ...
                              <MenuItem
                                onClick={() => {
                                  if (record.status !== 'success') {
                                    ...
                                    setActionMessage({
                                      tone: 'error',
                                      text: '80G receipts are only available for successful donations.',
                                    })
                                    return
                                  }
                                  void exportReceipt(record.id)
                                }}
                              >
                                {receiptBusyId === record.id ? 'Exporting receipt…' : 'Export receipt'}
                              </MenuItem>
