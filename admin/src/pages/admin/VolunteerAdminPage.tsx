...
export default function VolunteerAdminPage() {
  ...
  const handleAssignProject = async (input: { projectId: string; role: string; startsAt: string }) => {
    ...
    await assignVolunteerToProject({
      ...
      volunteerName: activeVolunteer.fullName,
      currentAssignedTeam: activeVolunteer.assignedTeam,
    })
    const data = await getVolunteerDashboardData()
    setDashboard(data)
    const refreshed = data.volunteers.find((v) => v.id === activeVolunteer.id)
    if (refreshed) {
      setActiveVolunteer(refreshed)
      setTeam(refreshed.assignedTeam ?? '')
    }
  }

  const handleAssignmentStatus = async (assignmentId: string, status: AssignmentStatus) => {
    await updateVolunteerAssignmentStatus(assignmentId, status)
    const data = await getVolunteerDashboardData()
    setDashboard(data)
    if (activeVolunteer) {
      const refreshed = data.volunteers.find((v) => v.id === activeVolunteer.id)
  ...
  const handleCreateTask = async (input: { title: string; dueDate: string; projectId?: string }) => {
    if (!activeVolunteer) return
  ...
  const handleTaskStatus = async (taskId: string, status: VolunteerTask['status']) => {
    await updateVolunteerTaskStatus(taskId, status)
    const data = await getVolunteerDashboardData()
    setDashboard(data)
    if (activeVolunteer) {
      const refreshed = data.volunteers.find((v) => v.id === activeVolunteer.id)
  ...
  const handleGenerateIdCards = async () => {
    ...
    const batch = approved.slice(0, 5)
    for (const volunteer of batch) {
      await downloadVolunteerIdCard(volunteer)
      await new Promise((resolve) => window.setTimeout(resolve, 350))
    }
    window.alert(`Downloaded ${batch.length} volunteer ID card${batch.length === 1 ? '' : 's'}.`)
  }
