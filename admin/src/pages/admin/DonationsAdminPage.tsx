...
import DonationsTable, { DEFAULT_DONATION_TABLE_FILTERS } from '../../components/admin/donations/DonationsTable'
...
export default function DonationsAdminPage() {
  ...
  const [tableFilters, setTableFilters] = useState<DonationTableFilters>(DEFAULT_DONATION_TABLE_FILTERS)
  ...
  /** Full filter-matching set (all pages) — used for export and selection. */
  const filteredDonations = useMemo(
    () => (dashboard ? filterDonations(dashboard.allDonations, tableFilters) : []),
  ...
  const handleDownloadReceipt = async (id: string) => {
    ...
    try {
      ...
      await downloadReceiptForDonationId(id)
      try {
        await markReceiptDownloaded(id)
        setActionMessage({ tone: 'ok', text: '80G receipt downloaded.' })
      } catch (markErr) {
        console.warn(markErr)
        setActionMessage({
          tone: 'error',
          text: 'Receipt PDF downloaded, but status could not be updated. Refresh and retry if needed.',
        })
      }
      await refresh()
    } catch (err) {
      setActionMessage({
        tone: 'error',
        text: err instanceof Error ? err.message : 'Could not download the 80G receipt.',
      })
    } finally {
    ...
    <>
      ...
      {dashboard ? (
        <button
          ...
          onClick={() => void exportDonationsCsv(filteredDonations)}
        >
  ...
  const selectedDonations = filteredDonations.filter((d) => selectedIds.has(d.id))
