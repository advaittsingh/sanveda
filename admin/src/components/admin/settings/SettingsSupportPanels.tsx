...
const ENV_CONFIG_HINT =
  'Configured via Vercel environment variables — secrets are not editable in this UI.'

/** Read-only control that looks inactive and explains why it is not clickable. */
function EnvManagedAction({ label }: { label: string }) {
  return (
    <button
      type="button"
      className={adminBtnSecondary}
      disabled
      title={ENV_CONFIG_HINT}
      aria-disabled="true"
    >
      {label}
    </button>
...
export function SettingsDonationsPanel({ donations, gateways, onChange, onSave }: DonProps) {
  return (
    <div className="space-y-6">
      ...
      <AdminCard>
        <h3 className="mb-2 text-base font-semibold text-[#0B2C6B]">Payment Gateway Settings</h3>
        <p className="mb-4 text-sm text-slate-500">{ENV_CONFIG_HINT}</p>
        <DataTable
          columns={[
            ...
            { key: 'actions', header: 'Action', render: (g) => (
              <EnvManagedAction label={g.status === 'connected' ? 'Configured' : 'Not connected'} />
            ) },
          ]}
          data={gateways}
          keyFn={(g) => g.id}
...
export function SettingsCertificatesPanel({ certificates }: { certificates: CertificateTemplate[] }) {
  return (
    <AdminCard>
      <h3 className="mb-2 text-base font-semibold text-[#0B2C6B]">Certificate Management</h3>
      <p className="mb-4 text-sm text-slate-500">
        Template editing is not available in the admin UI yet. Placeholders below are read-only.
      </p>
      <div className="space-y-4">
        {certificates.map((c) => (
          <div key={c.id} className="rounded-xl border border-[#E5E7EB] p-4">
            ...
            <div className="mt-2">
              <button
                ...
                className={adminBtnSecondary}
                disabled
                title="Certificate template editing is not available in the admin UI."
                aria-disabled="true"
              >
                Edit Template
              </button>
...
export function SettingsIntegrationsPanel({ integrations }: { integrations: IntegrationStatus[] }) {
  return (
    <AdminCard>
      <h3 className="mb-2 text-base font-semibold text-[#0B2C6B]">API & Integrations</h3>
      <p className="mb-4 text-sm text-slate-500">{ENV_CONFIG_HINT}</p>
      <DataTable
        columns={[
          ...
          { key: 'status', header: 'Status', render: (i) => <StatusBadge status={i.status === 'connected' ? 'active' : 'draft'} /> },
          { key: 'actions', header: 'Action', render: (i) => (
            <EnvManagedAction label={i.status === 'connected' ? 'Connected' : 'Not connected'} />
          ) },
        ]}
        data={integrations}
        keyFn={(i) => i.id}
...
export function SettingsBackupPanel({ backup }: { backup: SettingsDashboardData['backup'] }) {
  const opsUnavailable =
    'Managed via ops / not available in the admin UI. Backups are handled outside this panel.'

  return (
    <AdminCard>
      <h3 className="mb-2 text-base font-semibold text-[#0B2C6B]">Backup & Recovery</h3>
      <p className="mb-4 text-sm text-slate-500">{opsUnavailable}</p>
      <DataTable
        columns={[
        ...
        data={[
          ...
          { item: 'Last Backup', enabled: Boolean(backup.lastBackup && backup.lastBackup !== '—') },
          { item: 'Disaster Recovery', enabled: backup.disasterRecovery },
        ]}
        keyFn={(r) => r.item}
        emptyMessage=""
      />
      {backup.lastBackup && backup.lastBackup !== '—' ? (
        <p className="mt-2 text-xs text-slate-500">Last backup: {backup.lastBackup}</p>
      ) : null}
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" className={adminBtnPrimary} disabled title={opsUnavailable} aria-disabled="true">
          Create Backup
        </button>
        <button type="button" className={adminBtnSecondary} disabled title={opsUnavailable} aria-disabled="true">
          Restore Backup
        </button>
        <button type="button" className={adminBtnSecondary} disabled title={opsUnavailable} aria-disabled="true">
          Export Database
