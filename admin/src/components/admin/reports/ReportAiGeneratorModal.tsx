...
const OUTPUT_FORMATS = ['PDF', 'PowerPoint', 'Excel', 'Dashboard'] as const

export default function ReportAiGeneratorModal({ open, onClose, onGenerate }: Props) {
  const [prompt, setPrompt] = useState(
    'Generate annual healthcare impact report for FY 2025-26 including:\n- donors\n- expenses\n- beneficiaries\n- outcomes',
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
        <div className="mb-5 flex items-start justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-lg font-semibold text-[#0B2C6B]">
              <Sparkles size={20} className="text-violet-600" /> AI Report Generator
            </h2>
            <p className="text-sm text-slate-500">Describe the report you need — AI will compile data across modules</p>
          </div>
        ...
        <div>
          ...
          <textarea
            className={`${adminInputClass} min-h-[140px] resize-y`}
            value={prompt}
        ...
        <div className="mt-4">
          <label className={adminLabelClass}>Output Formats</label>
