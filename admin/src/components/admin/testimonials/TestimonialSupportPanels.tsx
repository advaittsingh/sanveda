...
export function TestimonialStoryBuilder() {
  ...
  return (
    <AdminCard>
      ...
      <p className="mt-1 text-sm text-slate-500">Build a story only from verified beneficiary records and documented outcomes.</p>
...
export function TestimonialSentimentPanel({ testimonials }: { testimonials: TestimonialProfile[] }) {
  ...
  const positiveCount = testimonials.filter((t) => t.sentiment === 'very_positive' || t.sentiment === 'positive').length
  const positivePct = testimonials.length ? Math.round((positiveCount / testimonials.length) * 100) : 0
  return (
    <AdminCard>
      ...
      {testimonials.length ? <p className="mt-4 text-center text-sm font-semibold text-emerald-700">Overall Sentiment Score: {positivePct}% Positive</p> : null}
    </AdminCard>
  )
}
