...
export default function DonationDetailDrawer({
  ...
  }, [donation?.id, donation?.notes, donation?.refundReason])
  ...
  useEffect(() => {
    ...
    // Build from the already-loaded ops row first (same path the table download uses),
    // then optionally enrich from donation_receipts without blocking the action buttons.
    const load = async () => {
      try {
        const local = await prepareReceipt80G(donation)
        if (cancelled) return
        setReceiptData(local)
        setReceiptLoading(false)

        const enriched = await getReceipt80GForDonationId(donation.id).catch(() => null)
        if (!cancelled && enriched) setReceiptData(enriched)
      } catch (err) {
        if (cancelled) return
        setReceiptData(null)
        setReceiptLoadError(err instanceof Error ? err.message : 'Could not load receipt.')
        setReceiptLoading(false)
      }
    }
              ...
              <section>
                ...
                {receiptLoading ? (
                  ...
                  <>
                    <Receipt80GActions
                      data={receiptData}
                      onView={() => setShowReceipt(true)}
                      onDownload={async () => {
                        await markReceiptDownloaded(donation.id)
                        await afterReceipt()
                      }}
                      onEmail={async () => {
                        await markReceiptEmailed(donation.id)
                        await afterReceipt()
                      }}
                      onRegenerate={async () => {
                  ...
                  <p className="text-sm text-red-700" role="alert">
                    {receiptLoadError || 'Receipt not available yet.'}
                  </p>
                )}
              </section>
            ) : null}
