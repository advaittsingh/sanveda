          ...
          <div className="space-y-5">
            ...
            <AdminCard>
              ...
              <div className="space-y-3">
                {dashboard.failedRenewals.map((subscriber) => (
                  <div key={subscriber.id} className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-medium text-[#0B2C6B]">{subscriber.donorName}</p>
                        <p className="text-xs text-slate-500">{formatIndianCompact(subscriber.amount)} due on {formatFullDate(subscriber.nextBillingDate)}</p>
                      </div>
            ...
            <AdminCard>
              ...
              <div className="space-y-3">
                {dashboard.topSubscribers.map((subscriber) => (
                  <div key={subscriber.name} className="flex items-center justify-between rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] px-4 py-3">
                    <div>
                      <p className="font-medium text-[#0B2C6B]">{subscriber.name}</p>
                      <p className="text-xs text-slate-500">{subscriber.planName}</p>
                    </div>
                    <p className="font-semibold text-emerald-700">{formatIndianCompact(subscriber.value)}</p>
                  </div>
                ))}
              </div>
            </AdminCard>
            ...
            <AdminCard>
              ...
              <div className="space-y-3">
                <MetricRow label="Next Month" value={formatIndianCompact(dashboard.forecast.nextMonth)} />
                <MetricRow label="Next Quarter" value={formatIndianCompact(dashboard.forecast.nextQuarter)} />
                <MetricRow label="Next Year" value={formatIndianCompact(dashboard.forecast.nextYear)} />
              </div>
            </AdminCard>
            ...
            {selectedSubscriber ? (
              <AdminCard>
                <div className="mb-4">
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">Subscriber profile, billing cadence, and stewardship notes</p>
                </div>
                <div className="space-y-3">
                  <MetricRow label="Donor" value={selectedSubscriber.donorName} />
                  <MetricRow label="Plan" value={selectedSubscriber.planId} />
                  <MetricRow label="Lifetime Value" value={formatIndianCompact(selectedSubscriber.lifetimeValue)} />
                  <MetricRow label="Start Date" value={formatFullDate(selectedSubscriber.startDate)} />
                  <MetricRow label="Next Billing" value={formatFullDate(selectedSubscriber.nextBillingDate)} />
                  <MetricRow label="Last Payment" value={formatFullDate(selectedSubscriber.lastPaymentDate)} />
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full rounded-xl border border-[#E5E7EB] bg-white px-3 py-2.5 text-sm text-slate-700 outline-none transition focus:border-[#0B2C6B]/30 focus:ring-2 focus:ring-[#0B2C6B]/10"
                    />
                  </div>
                  <div className="flex flex-wrap gap-2">
...
function MetricRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] px-4 py-3">
      ...
      <span className="text-sm font-semibold text-[#0B2C6B] capitalize">{value}</span>
    </div>
  )
}
