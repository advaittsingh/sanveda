...
interface Props {
  insights: VolunteerDashboardData['aiInsights']
}

export default function VolunteerAiInsights({ insights }: Props) {
  return (
    <AdminCard>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h3 className="text-base font-semibold text-[#0B2C6B]">AI Insights</h3>
          <p className="text-sm text-slate-500">Volunteer workforce intelligence</p>
        </div>
      ...
      <ul className="space-y-3">
        {insights.map((insight) => (
          <li
            key={insight.id}
            className={`rounded-xl border px-4 py-3 text-sm font-medium ${toneStyles[insight.tone]}`}
          >
            • {insight.message}
          </li>
        ))}
