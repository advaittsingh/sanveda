  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex flex-wrap gap-2">
        <button type="button" className={adminBtnPrimary} onClick={onAddMember}>
          <UserPlus size={15} className="mr-1.5" />
          Add Member
        </button>
        <button type="button" className={adminBtnSecondary} onClick={onApproveApplications}>
          <BadgeCheck size={15} className="mr-1.5" />
          Approve Applications
        </button>
        <button type="button" className={adminBtnSecondary} onClick={onGenerateCertificates}>
          <Award size={15} className="mr-1.5" />
          Generate Certificates
        </button>
        <button type="button" className={adminBtnSecondary} onClick={onSendRenewals}>
...
export function MembershipEmptyState({ onAddMember }: { onAddMember: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#E5E7EB] bg-[#F8FAFC] px-6 py-16 text-center">
      ...
      <h3 className="text-lg font-semibold text-[#0B2C6B]">No members yet</h3>
      <p className="mt-2 max-w-md text-sm text-slate-500">
        Membership applications from the public portal will appear here for review, approval, and certificate generation.
      </p>
      <div className="mt-6 flex flex-wrap justify-center gap-2">
        <button type="button" className={adminBtnPrimary} onClick={onAddMember}>
          Add Member
        </button>
        <Link to="/membership" className={adminBtnSecondary}>
          View Public Portal
        </Link>
      </div>
    </div>
