...
export default function InternshipToolbar({
  viewMode,
  onViewModeChange,
  onAddIntern,
  onImport,
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap gap-2">
          <button type="button" className={adminBtnPrimary} onClick={onAddIntern}>
          ...
          <button type="button" className={adminBtnSecondary} onClick={onAssignMentor}>
            Assign Mentor
          </button>
...
export function InternshipEmptyState({ onAddIntern }: { onAddIntern: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#E5E7EB] bg-[#F8FAFC] px-6 py-16 text-center">
      ...
      <h3 className="text-lg font-semibold text-[#0B2C6B]">No internship applications yet</h3>
      <p className="mt-2 max-w-md text-sm text-slate-500">
        Applications from the public internship portal will appear here for screening, mentoring, and lifecycle management.
      </p>
      <div className="mt-6 flex flex-wrap justify-center gap-2">
        <button type="button" className={adminBtnPrimary} onClick={onAddIntern}>
          Add Intern
        </button>
        <Link to="/internship/apply" className={adminBtnSecondary}>
          View Public Portal
        </Link>
      </div>
    </div>
