...
const FOLDER_LABEL = Object.fromEntries(DOCUMENT_FOLDERS.map((f) => [f.value, f.label])) as Record<
  DocumentFolder,
  string
>
...
function computeKpis(docs: DocumentProfile[]) {
  ...
  return {
    ...
    storageUsedGb: Math.round((storageMb / 1024) * 10) / 10,
  }
}
...
function computeAiInsights(docs: DocumentProfile[], alerts: ExpiryAlert[]) {
  if (docs.length === 0) {
    return [
      {
        id: 'empty',
        message: 'No documents yet. Upload compliance certificates, reports, and policies.',
        tone: 'info' as const,
      },
    ]
  }
  ...
  return expiringCount > 0
    ? [
        {
          id: 'expiry',
          message: `${expiringCount} document${expiringCount === 1 ? '' : 's'} expiring within 60 days`,
