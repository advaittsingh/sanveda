...
function enrichLegacyLog(log: AuditLog): AuditLogEntry {
  ...
  const isPublic = log.details?.source === 'public_submission' || log.details?.role === 'Public'
  return {
    id: log.id,
    userId: log.userId,
    userName: String(log.details?.user ?? log.details?.userId ?? (isPublic ? 'Public submission' : 'System')),
    role: String(log.details?.role ?? (isPublic ? 'Public' : 'Admin')),
    department: String(log.details?.department ?? (isPublic ? 'Public Intake' : 'Administration')),
    ...
    ip: String(log.ip ?? log.details?.ip ?? '—'),
    device: String(log.device ?? log.details?.device ?? '—'),
    browser: String(log.browser ?? log.details?.browser ?? '—'),
...
export async function getAuditDashboardData(): Promise<AuditDashboardData> {
  ...
  return {
    ...
    financialLogs,
    volunteerLogs: logs
      .filter((l) => l.entityType === 'volunteers' || l.entityType === 'internships')
      .slice(0, 5)
      .map((l) => ({
      id: l.id,
    ...
    membershipLogs: logs
      .filter((l) => l.entityType === 'memberships' || l.entityType === 'recurring_donations')
      .slice(0, 5)
      .map((l) => ({
      id: l.id,
      date: new Date(l.createdAt).toLocaleDateString('en-IN'),
