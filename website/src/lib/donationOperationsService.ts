...
function mapRecord(
  ...
  return {
    ...
    taxExemption: (meta?.taxExemption as TaxExemption | undefined) ?? 'None',
...
export async function approveDonation(id: string) {
  return withAudit('APPROVE', 'donations', id, async () => {
    ...
    if (donation.status !== 'completed') {
      throw new Error('Only Razorpay-verified payments can be approved as completed')
    }
...
/** Record that a receipt email was already delivered (does not send again). */
export async function markReceiptEmailed(id: string) {
...
export async function approveRefund(refundId: string, donationId: string) {
  return withAudit('REFUND_APPROVE', 'donations', donationId, async () => {
    await refundRazorpayPayment(refundId)
  })
}
...
/**
 * Export donations to CSV. Always exports the full provided list (not a table page).
