...
function AboutHeroBanner({
  ...
  return (
    <div className="about-banner" style={{ marginTop: tablet ? 20 : -42 }} aria-busy={Boolean(loading)}>
      {loading && slots.length === 0
        ? [0, 1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className={bannerClasses[i]}
              style={{ background: C.cream, minHeight: mobile ? 120 : 200, borderRadius: 12 }}
              aria-hidden
            />
          ))
        : slots.map((src, i) =>
            src ? (
              <div key={`${src}-${i}`} className={bannerClasses[i]}>
                <img src={src} alt={`About Sanveda highlight ${i + 1}`} style={maskImg(ABOUT_BANNER_MASKS[i])} />
              </div>
            ) : null,
          )}
    </div>
    ...
    <div style={{ background: C.white }}>
      ...
      <div style={{ width: '100%', paddingLeft: lg ? 0 : 40, paddingRight: lg ? 0 : 40 }}>
        ...
        <section style={{ marginBottom: mobile ? 30 : md ? 40 : 82, paddingLeft: lg ? 16 : 0, paddingRight: lg ? 16 : 0 }}>
          ...
          <AboutHeroBanner images={cms.heroImages} loading={cms.heroLoading} mobile={mobile} tablet={tablet} />
        </section>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: mobile ? 10 : 20 }}>
              ...
              {cms.visionPoint1 ? <VisionStatCard value={cms.visionPoint1} label={cms.visionPoint1Label} mobile={mobile} tablet={tablet} /> : null}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: mobile ? 10 : 20 }}>
              {cms.visionPoint2 ? <VisionStatCard value={cms.visionPoint2} label={cms.visionPoint2Label} mobile={mobile} tablet={tablet} /> : null}
        ...
        {/* Do not wrap in AnimatedSection — a tall opacity:0 block here
            created a blank scroll gap before the footer (BUG-002). */}
