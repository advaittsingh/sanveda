...
export default function VerifyPage() {
  ...
  const runVerify = async (input: string) => {
    ...
    const trimmed = normalizeVerifyInput(input)
    if (!trimmed) {
      setLoading(false)
      return
    }

    const receiptMatch = await verifyReceiptToken(trimmed)
    if (receiptMatch) {
    ...
    if (!record) {
      setError('No authentic document found for this verification code or ID.')
      setLoading(false)
      return
    }
    ...
    <div style={{ background: C.white, paddingBottom: mobile ? 40 : 80 }}>
      ...
      <div style={{ width: '94.44%', maxWidth: 560, margin: '0 auto', padding: mobile ? '32px 16px' : '48px 0' }}>
        ...
        <p style={{ color: C.textMuted, marginBottom: 28, lineHeight: 1.6 }}>
          Scan the QR code on an ID card, certificate, appointment letter, or 80G receipt — or paste the
          verification code / link here.
        </p>
        ...
        {receipt && (
          <div style={{ border: `2px solid #155724`, borderRadius: 16, padding: 24, background: '#d4edda22' }}>
            ...
            <p><strong>Amount:</strong> ₹{Number(receipt.amount).toLocaleString('en-IN')}</p>
            <p><strong>Date:</strong> {new Date(receipt.paidAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
            {receipt.taxEligible ? <p><strong>Tax eligibility:</strong> Recorded as eligible</p> : null}
