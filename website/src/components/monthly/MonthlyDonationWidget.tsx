...
export default function MonthlyDonationWidget({
  ...
  const onDonate = () => {
    ...
    if (!causeId) {
      setCauseError('Please select a cause to continue.')
    ...
    const params = new URLSearchParams({
      ...
      title: cause?.title ?? 'Monthly Donation',
      monthly: '1',
    })
    if (cause?.slug) params.set('slug', cause.slug)
    if (causeId) params.set('id', causeId)
  ...
  return (
    ...
    >
      ...
      <p style={{ fontWeight: 600, fontSize: s.labelSize, lineHeight: '100%', color: '#141414', margin: '0 0 8px' }}>
        Select Cause <span aria-hidden="true">*</span>
      </p>

      <div style={{ position: 'relative', marginBottom: causeError ? 6 : s.dropdownMb }}>
        <select
          ref={selectRef}
          value={causeId}
          onChange={(e) => {
          ...
          aria-describedby={causeError ? causeErrorId : undefined}
          required
          style={{
            width: '100%',
            height: s.dropdownH,
            ...
            paddingRight: 36,
            border: `${s.borderW}px solid ${causeError ? '#C0392B' : '#DDE2E5'}`,
            fontWeight: 500,
            fontSize: 14,
      ...
      <div
        style={{
          ...
          borderRadius: s.dropdownRadius,
          padding: `0 ${s.dropdownPadX}px`,
          border: `${s.borderW}px solid ${amountError ? '#C0392B' : 'rgba(14, 79, 168, 0.45)'}`,
          background: C.cream,
          marginBottom: amountError ? 6 : s.customMb,
          display: 'flex',
          alignItems: 'center',
          gap: 8,
        }}
